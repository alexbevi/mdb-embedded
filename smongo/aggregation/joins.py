...
def lookup_stage(
    ...
    if not all([from_coll, local_field, foreign_field, as_field]):
        missing = [k for k, v in {"from": from_coll, "localField": local_field, "foreignField": foreign_field, "as": as_field}.items() if not v]
    ...
    use_indexed = foreign_coll is not None and _foreign_has_index(foreign_coll, foreign_field)
    ...
    if foreign_coll is not None:
        try:
            foreign_docs = foreign_coll.get_all() if hasattr(foreign_coll, 'get_all') else list(foreign_coll.find({}))
        except (KeyError, AttributeError, TypeError):
            pass
...
def _bind_vars_in_expr(expr: Any, scope: dict[str, Any]) -> Any:
    if isinstance(expr, str) and expr.startswith("$$") and not expr.startswith("$$ROOT") and not expr.startswith("$$CURRENT"):
        var_name = expr[2:]
