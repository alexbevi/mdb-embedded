...
/// Legacy PyDict-based validation (retained as reference implementation).
///
/// Both `RustLocalCollection::validate_doc` and `py_validate_document` now
/// delegate to `smongo_engine::schema::validate_document` via BSON conversion.
#[allow(dead_code)]
...
/// Python-callable wrapper: delegates to `smongo_engine::schema` via BSON conversion.
///
/// The internal `validate_document` is retained as a fallback for any call sites
/// that haven't migrated yet, but the primary path now runs through the engine.
#[pyfunction]
