...
pub struct WtSession {
    raw: *mut WT_SESSION,
    /// When set, `open_sibling_session` for `smongo_engine::StorageSession` can open another session.
    conn: Option<*mut WT_CONNECTION>,
}
...
impl WtSession {
    ...
    /// Like [`from_raw`], but keeps the connection pointer so additional sessions can be opened.
    pub unsafe fn from_raw_with_conn(raw: *mut WT_SESSION, conn: *mut WT_CONNECTION) -> Self {
        Self {
            raw,
            conn: Some(conn),
        }
    ...
    pub fn connection_ptr(&self) -> Option<*mut WT_CONNECTION> {
        self.conn
    }
    ...
    pub fn create(&self, name: &str, config: &str) -> WtResult<()> {
        ...
        let create_fn = unsafe { (*self.raw).create }.ok_or_else(|| WtError {
            code: -1,
            message: "create vtable null".into(),
        })?;
    ...
    pub fn drop_table(&self, name: &str, config: Option<&str>) -> WtResult<()> {
        ...
        let drop_fn = unsafe { (*self.raw).drop }.ok_or_else(|| WtError {
            code: -1,
            message: "drop vtable null".into(),
        })?;
    ...
    pub fn open_cursor(&self, uri: &str, config: Option<&str>) -> WtResult<WtCursor> {
        ...
        let c_config = optional_cstr(config)?;
        let config_ptr = c_config.as_ref().map_or(ptr::null(), |c| c.as_ptr());
        let open_cursor_fn = unsafe { (*self.raw).open_cursor }.ok_or_else(|| WtError {
            code: -1,
            message: "open_cursor vtable null".into(),
        })?;
    ...
    /// Open another session on the same connection (for `StorageSession::open_sibling_session`).
    pub fn open_sibling(&self) -> WtResult<WtSession> {
        let conn = self.conn.ok_or_else(|| WtError {
            code: -1,
            message: "WtSession has no connection handle for sibling session".into(),
        })?;
        super::open_session_from_conn_ptr(conn)
    }

    pub fn checkpoint(&self, config: Option<&str>) -> WtResult<()> {
        let c_config = optional_cstr(config)?;
        let config_ptr = c_config.as_ref().map_or(ptr::null(), |c| c.as_ptr());
        let cp_fn = unsafe { (*self.raw).checkpoint }.ok_or_else(|| WtError {
            code: -1,
            message: "checkpoint vtable null".into(),
        })?;
    ...
    pub fn begin_transaction(&self, config: Option<&str>) -> WtResult<()> {
        let c_config = optional_cstr(config)?;
        let config_ptr = c_config.as_ref().map_or(ptr::null(), |c| c.as_ptr());
        let begin_fn = unsafe { (*self.raw).begin_transaction }.ok_or_else(|| WtError {
            code: -1,
            message: "begin_transaction vtable null".into(),
        })?;
        // SAFETY: raw is a valid WT_SESSION; config_ptr is valid or null.
    ...
    pub fn commit_transaction(&self, config: Option<&str>) -> WtResult<()> {
        let c_config = optional_cstr(config)?;
        let config_ptr = c_config.as_ref().map_or(ptr::null(), |c| c.as_ptr());
        let commit_fn = unsafe { (*self.raw).commit_transaction }.ok_or_else(|| WtError {
            code: -1,
            message: "commit_transaction vtable null".into(),
        })?;
        // SAFETY: raw is a valid WT_SESSION; config_ptr is valid or null.
    ...
    pub fn rollback_transaction(&self, config: Option<&str>) -> WtResult<()> {
        let c_config = optional_cstr(config)?;
        let config_ptr = c_config.as_ref().map_or(ptr::null(), |c| c.as_ptr());
        let rollback_fn = unsafe { (*self.raw).rollback_transaction }.ok_or_else(|| WtError {
            code: -1,
            message: "rollback_transaction vtable null".into(),
        })?;
        // SAFETY: raw is a valid WT_SESSION; config_ptr is valid or null.
    ...
    pub fn verify(&self, name: &str, config: Option<&str>) -> WtResult<()> {
        ...
        let c_config = optional_cstr(config)?;
        let config_ptr = c_config.as_ref().map_or(ptr::null(), |c| c.as_ptr());
        let verify_fn = unsafe { (*self.raw).verify }.ok_or_else(|| WtError {
            code: -1,
            message: "verify vtable null".into(),
        })?;
        // SAFETY: raw is a valid WT_SESSION; c_name/config_ptr are valid or null.
    ...
    pub fn compact(&self, name: &str, config: Option<&str>) -> WtResult<()> {
        ...
        let c_config = optional_cstr(config)?;
        let config_ptr = c_config.as_ref().map_or(ptr::null(), |c| c.as_ptr());
        let compact_fn = unsafe { (*self.raw).compact }.ok_or_else(|| WtError {
            code: -1,
            message: "compact vtable null".into(),
        })?;
        // SAFETY: raw is a valid WT_SESSION; c_name/config_ptr are valid or null.
    ...
    pub fn close(&mut self) -> WtResult<()> {
        if !self.raw.is_null() {
            let close_fn = unsafe { (*self.raw).close }.ok_or_else(|| WtError {
                code: -1,
                message: "session close vtable null".into(),
            })?;
