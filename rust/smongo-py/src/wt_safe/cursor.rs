...
use wiredtiger_sys::{
    wt_shim_get_key_raw, wt_shim_get_key_str, wt_shim_get_value_raw, wt_shim_get_value_str,
    wt_shim_set_key_raw, wt_shim_set_key_str, wt_shim_set_value_raw, wt_shim_set_value_str,
...
pub struct WtCursor {
    pub(crate) raw: *mut WT_CURSOR,
    pub(crate) _key_buf: Option<CString>,
    pub(crate) _val_buf: Option<CString>,
}

// SAFETY: Cursor is session-local; smongo_engine only uses it from one thread
// (collection lock). PyO3 and StorageCursor require Send.
unsafe impl Send for WtCursor {}
...
impl WtCursor {
    ...
    /// Set key from raw bytes (`key_format=u`).
    pub fn set_key_raw(&mut self, data: &[u8]) {
        let item = WT_ITEM {
        ...
        let fn_ptr = unsafe { (*self.raw).set_key };
        unsafe { wt_shim_set_key_raw(fn_ptr, self.raw, &item) };
        self._key_buf = None;
    }
    ...
    pub fn get_key_raw(&self) -> WtResult<Vec<u8>> {
        ...
        let fn_ptr = unsafe { (*self.raw).get_key };
        let rc = unsafe { wt_shim_get_key_raw(fn_ptr, self.raw, &mut item) };
        check(rc)?;
        let key = unsafe { std::slice::from_raw_parts(item.data as *const u8, item.size) }.to_vec();
        Ok(key)
    }
