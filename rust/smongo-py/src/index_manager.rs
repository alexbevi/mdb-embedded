...
use crate::wt_bridge::{RustWtSession, WtResultExt};
...
impl IndexDef {
    ...
    pub(crate) fn to_py_dict(&self, py: Python<'_>) -> PyResult<Py<PyDict>> {
        ...
        let key_tuples: Vec<Bound<'_, pyo3::types::PyTuple>> = if self.index_type == IndexType::TwoDsphere {
            let (f, _) = self
                .keys
                .first()
                .ok_or_else(|| PyRuntimeError::new_err("2dsphere index missing field"))?;
            vec![pyo3::types::PyTuple::new(
                py,
                [f.into_pyobject(py)?.into_any(), pyo3::types::PyString::new(py, "2dsphere").into_any()],
            )?]
        } else {
            self.keys
                .iter()
...
impl RustIndexManager {
    ...
    fn insert_btree_entry(
        ...
        let mut cursor = session.open_cursor(table_uri, Some("overwrite=true")).py()?;
    ...
    fn insert_entry(&self, py: Python<'_>, idx: &IndexDef, doc: &Bound<'_, PyAny>) -> PyResult<()> {
        ...
        match idx.index_type {
            ...
            // 2dsphere keys live in smongo-engine index tables only (no WiredTiger shadow index).
            IndexType::TwoDsphere => Ok(()),
        }
    }
    ...
    pub(crate) fn create_index(
        ...
        if idx_type == IndexType::TwoDsphere {
            if parsed_keys.len() != 1 {
                return Err(PyValueError::new_err(
                    "2dsphere index must be defined on exactly one field",
                ));
            }
        }
        ...
        if idx_type == IndexType::TwoDsphere && unique {
            return Err(PyValueError::new_err(
                "unique 2dsphere indexes are not supported",
            ));
        }
        if idx_type == IndexType::TwoDsphere {
            return Err(PyValueError::new_err(
                "2dsphere indexes must be created with RustLocalCollection.create_index (engine-backed)",
            ));
        }
        ...
        let session = self.borrow_session()?;
        session.create(&table_uri, "key_format=S,value_format=S").py()?;
        ...
        let mut meta_cursor = session.open_cursor(&self.meta_uri, Some("overwrite=true")).py()?;
        meta_cursor.set_key_str(&name);
        meta_cursor.set_value_str(&json_str);
        meta_cursor.update().py()?;
        meta_cursor.close().py()?;
    ...
    pub(crate) fn drop_index(&mut self, py: Python<'_>, name: &str) -> PyResult<()> {
        ...
        let mut cursor = session.open_cursor(&self.meta_uri, Some("overwrite=true")).py()?;
    ...
    /// Register `2dsphere` in planner metadata only. Index entries are maintained by `smongo-engine`.
    pub(crate) fn register_2dsphere_planner_metadata(
        &mut self,
        py: Python<'_>,
        name: &str,
        keys: &Bound<'_, PyAny>,
        ...
        let parsed_keys: Vec<(String, IndexDir)> = keys_list
            .iter()
            .map(|(f, _)| (f.clone(), IndexDir::Asc))
            .collect();
        ...
        let key_tuples: Vec<Bound<'_, pyo3::types::PyTuple>> = keys_list
            ...
            .map(|(field, d)| {
                pyo3::types::PyTuple::new(
                    py,
                    [PyString::new(py, field).into_any(), d.bind(py).clone()],
                )
            })
            .collect::<PyResult<_>>()?;
        let keys_py = PyList::new(py, key_tuples)?;
        ...
        if let Some(e) = expire {
            defn.set_item("expireAfterSeconds", e)?;
        }
        defn.set_item("type", "2dsphere")?;
        if let Some(ref pf) = partial_filter {
            defn.set_item("partialFilterExpression", pf.bind(py))?;
        }
        let json_str: String = py
        ...
        let idx = IndexDef {
            ...
            sparse,
            expire_after_seconds: expire,
            index_type: IndexType::TwoDsphere,
            partial_filter,
            table_uri: None,
        };
        self._indexes.insert(name.to_string(), idx);
        Ok(())
    }
    ...
    fn load_metadata(&mut self, py: Python<'_>) -> PyResult<()> {
        let session = self.borrow_session()?;
        let mut cursor = session.open_cursor(&self.meta_uri, None).py()?;
        while cursor.next().is_ok() {
            let name = cursor.get_key_str().py()?;
            let defn_json = cursor.get_value_str().py()?;
            let json_mod = crate::cached_modules::json_mod(py)?;
            ...
            for pair in keys_obj.try_iter()? {
                ...
                let dir = if let Ok(s) = d.extract::<String>() {
                    if s == "2dsphere" || s == "2d" {
                        IndexDir::Asc
                    } else {
                        IndexDir::Asc
                    }
                } else if let Ok(i) = d.extract::<i32>() {
                    if i == -1 {
                        IndexDir::Desc
                    } else {
                        IndexDir::Asc
            ...
            let table_uri = if idx_type == IndexType::TwoDsphere {
                None
            } else {
                Some(format!(
...
impl RustIndexManager {
    ...
    pub fn new(
        ...
        // Create the metadata table
        {
            ...
            s.create(&meta_uri, "key_format=S,value_format=S").py()?;
        }
