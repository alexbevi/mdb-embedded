...
//! ## Client type duality
//!
//! `ConnectionContext.local_client` can hold either:
//! - **`RedbLocalClient`** (Rust native) — created by `WireServer` and test fixtures.
//! - **`RedbClient`** (Python wrapper) — created by `MongoClient("local://...")` and
//!   `SyncManager`.
//!
//! The helper methods `local_client_typed()` and `get_db_typed()` transparently
//! unwrap the Python wrapper when present.  This fallback path is intentional
//! and must be kept as long as `MongoClient` and `SyncManager` construct
//! `RedbClient` instances.
...
/// require a `py.import()` on every command.  Built once per server
/// lifetime, shared across all connections via `Arc`.
///
/// `from_python` is generated by the `PythonImports` derive macro.
/// It imports `smongo._ffi_manifest` and resolves each field via `getattr`.
/// If you add a field, update `smongo/_ffi_manifest.py` to match.
#[derive(smongo_macros::PythonImports)]
#[py(module = "smongo._ffi_manifest")]
pub(crate) struct CachedImports {
    #[py(attr = "_USER_STORE")]
    pub user_store: Py<PyAny>,
    #[py(attr = "_USER_STORE_LOCK")]
    pub user_store_lock: Py<PyAny>,
    #[py(attr = "audit_mod")]
    pub audit_mod: Py<PyAny>,
    #[py(attr = "_TOPOLOGY_PROCESS_ID")]
    pub topology_pid: Py<PyAny>,
    #[py(attr = "_GIT_VERSION")]
    pub git_version: Py<PyAny>,
    #[py(attr = "_SERVER_START")]
...
impl ConnectionContext {
    pub(crate) fn cached_imports(&self) -> PyResult<&CachedImports> {
        self.cached.as_deref().ok_or_else(|| {
            PyRuntimeError::new_err(
                "CachedImports not initialized -- dispatch() has not run on this \
                 context yet (lazy init happens on first rs_dispatch call)",
            )
        })
    }

    /// Extract the underlying `RedbLocalClient`.
    ///
    /// Fast path: `local_client` is already `RedbLocalClient` (WireServer, tests).
    ...
    /// Get a `RedbLocalDB` handle, with per-connection caching.
    ///
    /// Fast path: `local_client` is `RedbLocalClient` — call `get_db()` directly.
    /// Slow path: `local_client` is Python `RedbClient` — call `get_db()` and
    /// unwrap the returned `RedbDB` via `_rust_db` attribute.
    pub(crate) fn get_db_typed(&self, py: Python<'_>, db_name: &str) -> PyResult<Py<RedbLocalDB>> {
        ...
        let lc = self.local_client.bind(py);
        let db: Py<RedbLocalDB> = if let Ok(rs_client) = lc.cast::<RedbLocalClient>() {
            rs_client.borrow().get_db(py, db_name)?
        } else {
            let py_db = lc.call_method1("get_db", (db_name,))?;
