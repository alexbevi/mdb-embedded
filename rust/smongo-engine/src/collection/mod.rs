...
use crate::index::{
    extract_index_key, generate_index_name, is_2dsphere_keys, validate_custom_index_name,
    IndexOptions, IndexSpec,
};
use crate::oplog::{append_oplog, CollectionOplogSettings};
...
enum FindCursorState<C: StorageCursor> {
    ...
    /// Precomputed matches (geospatial `$or` unions and `$near` sort).
    Materialized {
        docs: Vec<Document>,
        next_ix: usize,
    },
...
impl<S: StorageSession> Collection<S> {
    ...
    fn execute_plan(&self, plan: &ExecutionPlan, filter: &Document) -> CollectionResult<Vec<Document>> {
        match plan {
    ...
    pub fn drop_index(&self, index_name: &str) -> CollectionResult<()> {
        if index_name == "*" {
            ...
            for spec in indexes {
                if spec.name != "_id_" {
                    // Don't drop the implicit _id index
    ...
    fn insert_into_indexes(&self, doc: &Document) -> CollectionResult<()> {
        ...
        for index_spec in indexes {
            ...
            // Combine index key + _id for the key
            let mut combined_key = index_key_bytes.clone();
            combined_key.extend_from_slice(id_str.as_bytes());
    ...
    fn remove_from_indexes(&self, doc: &Document) -> CollectionResult<()> {
        ...
        for index_spec in indexes {
            ...
            let combined_key: Vec<u8> = if is_2dsphere_keys(&index_spec.keys) {
                #[cfg(target_arch = "wasm32")]
                {
                    continue;
                }
                #[cfg(not(target_arch = "wasm32"))]
                {
                    let Some(k) = twodsphere_index_key(doc, &index_spec.keys) else {
                        continue;
                    };
                    k
                }
