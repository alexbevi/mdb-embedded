...
#[allow(dead_code)]
fn py_to_bson(val: &Bound<'_, PyAny>) -> PyResult<Bson> {
    let py = val.py();
...
#[allow(dead_code)]
fn bson_to_py<'py>(py: Python<'py>, val: &Bson) -> PyResult<Bound<'py, PyAny>> {
    match val {
...
#[allow(dead_code)]
fn array_to_pylist<'py>(py: Python<'py>, arr: &[Bson]) -> PyResult<Bound<'py, PyList>> {
