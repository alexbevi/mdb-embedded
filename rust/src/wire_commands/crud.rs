...
fn cmd_find(
    ...
    let docs = coll_py
        .bind(py)
        .borrow()
        .find_streaming_typed(py, Some(filter_dict.as_any()))?;
...
fn cmd_distinct(
    ...
    let docs = coll_py
        .bind(py)
        .borrow()
        .find_streaming_typed(py, Some(&raw_query))?;
    ...
    let (size, n_objects, _estimate) = if kp_truthy && (min_key.is_some() || max_key.is_some()) {
        ...
        let doc_iter = if !query.is_empty() {
            coll_py
                .bind(py)
                .borrow()
                .find_streaming_typed(py, Some(query.as_any()))?
        } else {
            coll_py.bind(py).borrow().find_streaming_typed(py, None)?
        };
        let mut size: i64 = 0;
        let mut count: i64 = 0;
