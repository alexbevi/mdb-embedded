...
impl RustStreamingCursor {
    ...
    pub(crate) fn from_collection(
        py: Python<'_>,
        coll: &crate::local_collection::RustLocalCollection,
        ...
        let session_py: Py<PyAny> = match crate::transaction::get_active_txn_session(py) {
            Ok(Some(s)) => s,
            _ => coll.session_py.clone_ref(py).into_any(),
        };
        ...
        Py::new(
            py,
            Self {
                table_uri: coll.table_uri.clone(),
                query: q,
                rwlock: Arc::clone(&coll.rwlock),
                mutex: Arc::clone(&coll.mutex),
                planner: coll.planner.clone_ref(py),
                session_py,
    ...
    fn init_pk_lookup(
        ...
        let mut cursor = session.open_cursor(&self.table_uri, None)?;
    ...
    fn init_index_scan_typed(
        ...
        let session = self.borrow_session()?;
        let cursor = session.open_cursor(&self.table_uri, None)?;
