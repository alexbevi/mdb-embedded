...
impl From<WtError> for PyErr {
    fn from(e: WtError) -> PyErr {
        PyRuntimeError::new_err(format!("WiredTiger error {}: {}", e.code, e.message))
    }
}
...
/// Python-facing WiredTiger session -- thin PyO3 bridge over [`wt_safe::WtSession`].
#[pyclass]
...
impl RustWtSession {
    ...
    fn drop_table(&self, name: &str, config: Option<&str>) -> PyResult<()> {
        self.get()?.drop_table(name, config)?;
        Ok(())
    }
    ...
    fn compact(&self, name: &str, config: Option<&str>) -> PyResult<()> {
        self.get()?.compact(name, config)?;
        Ok(())
    }

    #[pyo3(signature = (name, config=None))]
    fn verify(&self, name: &str, config: Option<&str>) -> PyResult<()> {
        self.get()?.verify(name, config)?;
        Ok(())
    }
...
/// Python-facing WiredTiger cursor -- thin PyO3 bridge over [`wt_safe::WtCursor`].
#[pyclass]
