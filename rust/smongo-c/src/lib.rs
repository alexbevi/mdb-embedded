...
pub unsafe extern "C" fn smongo_create_index(
    ...
    // Custom index names are not yet supported by the engine — the name is
    // auto-generated.  We still accept and validate the parameter for forward
    // compatibility.
    if !name.is_null() {
        if let Err(code) = cstr_to_str(name) {
            return code;
        }
    }
...
pub unsafe extern "C" fn smongo_aggregate(
    ...
    // Convert the BSON "array document" (keys "0","1",...) into Vec<Document>
    let mut stages: Vec<(usize, Document)> = Vec::new();
    for (key, value) in pipeline_doc.iter() {
        let idx: usize = match key.parse() {
            Ok(i) => i,
            Err(_) => {
                set_last_error(&format!(
                    "pipeline must be a BSON array; unexpected key: {}",
                    key
                ));
                return SMONGO_ERROR_INVALID_BSON;
            }
        };
        let stage_doc = match value.as_document() {
            Some(d) => d.clone(),
            None => {
                set_last_error(&format!("pipeline stage {} is not a document", key));
                return SMONGO_ERROR_INVALID_BSON;
            }
        };
        stages.push((idx, stage_doc));
    }
    stages.sort_by_key(|(idx, _)| *idx);
    let pipeline_vec: Vec<Document> = stages.into_iter().map(|(_, d)| d).collect();
