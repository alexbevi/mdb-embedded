...
def _run(sensors) -> None:
    ...
    print("── seeding 10,000 sensor readings ──")
    docs = [
        {
            "_id": f"r_{i:05d}",
    ...
    print(f"  time: {elapsed:.2f}ms (1 BSON decode, not 500)\n")
    ...
    # compare with materializing everything
    t0 = time.perf_counter()
    ...
    # ── find().sort().skip().limit(): sort forces materialization ──
    print("── sort + skip + limit (sort requires full materialization) ──")
    t0 = time.perf_counter()
