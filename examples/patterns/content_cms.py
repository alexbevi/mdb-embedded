        ...
        [
            {
                "_id": "auth_1",
                "name": "Sarah Chen",
                "bio": "Staff engineer writing about distributed systems",
                "twitter": "@sarahchen",
            },
            {
                "_id": "auth_2",
                "name": "Marcus Johnson",
                "bio": "DevRel lead, conference speaker",
                "twitter": "@marcusj",
            },
            {
                "_id": "auth_3",
                "name": "Priya Gupta",
                "bio": "Database internals and storage engines",
                "twitter": "@priyaDB",
            },
        ]
    )
...
def queries(db) -> None:
    ...
    for d in articles.find({"title": {"$regex": "index", "$options": "i"}, "status": "published"}):
        print(f"  {d['title']}")
        print(
            f"    {d['views']:,} views  {d['likes']} likes  {d['meta']['reading_time_min']}min read"
        )
    ...
    for r in results:
        print(
            f"  {r['name']:20s}  {r['articles']} articles  "
    ...
    print(
        f"  Published: {stats['total_articles']} articles, {stats['total_words']:,} total words, "
