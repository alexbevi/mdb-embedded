...
def test_device_scoped_push(tmp_path, mongo_uri, remote_client, db_name):
    ...
    client = EmbeddedClient(f"local://{tmp_path}/fleet_push")
    mgr = SyncManager(
        client,
        mongo_uri,
        sync_config={
...
def test_device_scoped_pull(tmp_path, mongo_uri, remote_client, db_name):
    ...
    client = EmbeddedClient(f"local://{tmp_path}/fleet_pull")
    mgr = SyncManager(
        client,
        mongo_uri,
        sync_config={
...
def test_two_devices_isolated(tmp_path, mongo_uri, remote_client, db_name):
    ...
    client_a = EmbeddedClient(f"local://{tmp_path}/dev_a")
    client_b = EmbeddedClient(f"local://{tmp_path}/dev_b")
...
def test_time_windowed_sync(tmp_path, mongo_uri, remote_client, db_name):
    ...
    client = EmbeddedClient(f"local://{tmp_path}/time_win")
    mgr = SyncManager(
        client,
        mongo_uri,
        sync_config={
...
def test_node_id_in_oplog(tmp_path, mongo_uri):
    ...
    client = EmbeddedClient(f"local://{tmp_path}/oplog_nid")
    mgr = SyncManager(
        client,
        mongo_uri,
        sync_config={
    ...
    entries = [entry for _key, entry in reader.read_from(None, skip_internal=False)]
