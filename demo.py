...
def main():
    ...
    client = MongoClient("local://demo_wt_data", sync=sync_uri) if sync_uri else MongoClient("local://demo_wt_data")
    db = client["demo"]
    try:
    ...
    for doc in users.find({"dept": "engineering"}).sort("salary", -1).skip(1).limit(2).projection({"name": 1, "salary": 1, "_id": 0}):
        print(f"      {doc}")
    ...
    users.update_one({"name": "Diana"}, {"$rename": {"city": "location"}, "$currentDate": {"updatedAt": True}})
    ...
    result = users.bulk_write([
        InsertOne({"name": "Zara", "age": 26, "city": "BOS", "dept": "data", "salary": 115000, "tags": ["python"]}),
    ...
    stream = users.watch()
    users.insert_one({"name": "TTL user", "age": 29, "city": "NYC", "dept": "engineering", "salary": 100000, "expiresAt": time.time() - 10})
    ...
    if client.sync:
        ...
        print(f"    state={status['state']}  pushed={status['pushed']}  pulled={status['pulled']}  conflicts={status['conflicts']}")
        client.sync.pull()
        status = client.sync.status()
